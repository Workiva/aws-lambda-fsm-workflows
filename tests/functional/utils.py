...
class AWSStub(object):
    ...
    def __init__(self):
        ...
        self.primary_chaos = 0.0
        self.secondary_chaos = 0.0

        self.errors = Messages()

    def _chaos(self, primary):
        chaos = {True: self.primary_chaos, False: self.secondary_chaos}[primary]
        if chaos > 0.0:
            if random.uniform(0.0, 1.0) < chaos:
                raise ClientError({"Error": {}}, "chaos")

    def _lease_chaos(self, primary):
        # lease acquisition has own chaos
        chaos = {True: self.primary_chaos, False: self.secondary_chaos}[primary]
        if chaos > 0.0:
            if random.uniform(0.0, 1.0) < chaos:
                return False
...
class BaseFunctionalTest(unittest.TestCase):
    ...
    def _execute(self, aws, machine_name, context,
                 fail_primary_stream_stream=False,
                 fail_primary_stream_retry=False,
                 primary_chaos=0.0,
                 secondary_chaos=0.0):
        ...
        if primary_chaos:
            aws.primary_chaos = primary_chaos
        if secondary_chaos:
            aws.secondary_chaos = secondary_chaos
